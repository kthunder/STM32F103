...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitConfig)
{
	...
	currentmode = ((u32)GPIO_InitConfig->GPIO_Mode) & ((u32)0x0F);

	// bit4是 1表示输出，bit4是 0则是输入
	// 判断 bit4是 1还是 0，即首选判断是输入还是输出模式
	if ((((u32)GPIO_InitConfig->GPIO_Mode) & ((u32)0x10)) != 0x00)
	{
		// 输出模式则要设置输出速度
		currentmode |= (u32)GPIO_InitConfig->GPIO_Speed;
	}
	/*-----GPIO CRL 寄存器配置 CRL寄存器控制着低 8位 IO- ----*/
	// 配置端口低 8位，即 Pin0~Pin7
	if (((u32)GPIO_InitConfig->GPIO_Pin & ((u32)0x00FF)) != 0x00)
	{
		// 先备份 CRL寄存器的值
		tmpreg = GPIOx->CRL;
